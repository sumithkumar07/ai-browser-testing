...
export class DatabaseService {
  ...
  private async createIndexes(): Promise<void> {
    ...
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_agent_memory_importance ON agent_memory(importance)');
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_agent_performance_agent ON agent_performance(agent_id)');
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_agent_performance_task ON agent_performance(task_type)');
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_background_tasks_status ON background_tasks(status)');
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_background_tasks_priority ON background_tasks(priority)');
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_agent_learning_agent ON agent_learning(agent_id)');
  }

  // Bookmark Operations
  async saveBookmark(bookmark: BookmarkEntry): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  async getBookmarks(limit: number = 100): Promise<BookmarkEntry[]> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  async saveAgentMemory(memory: AgentMemoryEntry): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  async savePerformanceMetric(metric: AgentPerformanceMetric): Promise<void> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  async getPerformanceMetrics(agentId: string, limit: number = 100): Promise<AgentPerformanceMetric[]> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  async getBackgroundTasks(status?: string, limit: number = 100): Promise<BackgroundTask[]> {
    if (!this.db) throw new Error('Database not initialized');
